...
// 2. Cập nhật Interface: Nhận cả tên và link
interface NavItem {
  name: string;
  link: string;
}
...
interface SparkleNavbarProps {
  /**
   * Mảng chứa thông tin menu
   * @example [{ name: 'Trang chủ', link: '/' }, { name: 'Sản phẩm', link: '/san-pham' }]
   */
  items: NavItem[];
  color?: string;
}
...
const SparkleNavbar: React.FC<SparkleNavbarProps> = ({
  ...
  // Đổi kiểu Ref từ Button sang Anchor (thẻ a) vì Link render ra thẻ a
  const buttonRefs = useRef<(HTMLAnchorElement | null)[]>([]);

  // 3. Effect: Tự động cập nhật activeIndex dựa trên URL hiện tại
  useEffect(() => {
    const currentPath = location.pathname;
    const foundIndex = items.findIndex((item) => item.link === currentPath);
    
    // Nếu tìm thấy link trùng khớp, set active index
    if (foundIndex !== -1) {
      setActiveIndex(foundIndex);
    } else {
        // Xử lý trường hợp đặc biệt: Nếu không khớp chính xác, có thể check xem nó có chứa path không
        // Ví dụ: đang ở /product/123 thì vẫn active tab /product
        // Tạm thời để mặc định là 0 hoặc giữ nguyên
    }
  ...
  return (
    <>
      <style>{`
        ...
        .navigation-menu ul li a { /* Đổi selector từ button thành a */
          ...
          padding: 0;
          margin: 0;
          font-size: 22px; /* Đã sửa list-style thành font-size nếu đó là ý bạn, hoặc giữ nguyên nếu là custom */
